(function () {
  ...
  function initQuickLauncher(cfg: any) {
    ...
    function renderResults(items: any[], query: string) {
      ...
      let cmd: any = null;
      if (q) {
        try { cmd = parseBasicCommand(q, cfg); } catch (_) { cmd = null; }
      }
      ...
      currentResults.forEach(function (it, i) {
        ...
        const meta = document.createElement('span'); meta.className = 'ql-meta'; meta.textContent = it.type === 'go' ? 'go/' : it.type === 'command' ? 'cmd' : it.type === 'learned' ? '🧠' : (it.section || ''); (meta as HTMLElement).style.opacity = '0.7'; (meta as HTMLElement).style.fontSize = '12px';
  ...
  function learnCommandPattern(command: string) {
    if (!((config as any).analytics && (config as any).analytics.enableLocal)) return;
    ...
    try {
      ...
      for (let len = 2; len <= Math.min(5, command.length - 1); len++) {
        ...
        let existing = patterns[partial].find((p: any) => p.command === command);
        if (existing) {
          existing.count++;
  ...
  function readCommandPatterns(): any {
    try {
  ...
  function getPatternBoost(query: string, command: string): number {
    if (!((config as any).analytics && (config as any).analytics.enableLocal)) return 0;
    ...
    try {
      ...
      for (let len = 2; len <= Math.min(queryLower.length, 5); len++) {
        ...
        if (patternList) {
          const match = patternList.find((p: any) => p.command === command);
          if (match) {
  ...
  function getLearnedCommandSuggestions(query: string, cfg: any): any[] {
    if (!((config as any).analytics && (config as any).analytics.enableLocal)) return [];
    ...
    try {
      ...
      for (let len = 2; len <= Math.min(queryLower.length, 5); len++) {
        ...
        if (patternList) {
          patternList.forEach((pattern: any) => {
  ...
  function parseBasicCommand(input: string, cfg: any): any {
  ...
  function matchSimpleTemplate(template: string, input: string): any {
    // Convert template to regex: 'gh {owner}/{repo}' -> /^gh\s+([^\/\s]+)\/([^\s]+)$/
    const paramRegex = /\{([^}]+)\}/g;
    const params: string[] = [];
    let regexStr = template.replace(paramRegex, (match, paramName) => {
      params.push(paramName);
      if (paramName === 'q') return '(.+)'; // Query params can contain spaces
  ...
  function interpolateBasicUrl(template: string, params: any): string {
    let result = template;
